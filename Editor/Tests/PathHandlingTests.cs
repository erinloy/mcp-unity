...
namespace McpUnity.Tests
{
    ...
    public class PathHandlingTests
    {
        ...
        public void ValidateServerPath_WithValidPath_ReturnsTrue()
        {
            ...
            File.WriteAllText(Path.Combine(_tempDir, "UnityMcp.csproj"), "<Project />");

            // Act
            bool result = McpUtils.ValidateServerPath(_tempDir);

            // Assert
            Assert.IsTrue(result, "ValidateServerPath should return true for valid path with UnityMcp.csproj");
        }
        ...
        public void ValidateServerPath_WithSpacesInPath_ReturnsTrue()
        {
            ...
            File.WriteAllText(Path.Combine(_tempDirWithSpaces, "UnityMcp.csproj"), "<Project />");
        ...
        [Test]
        public void ValidateServerPath_WithMissingServerProject_ReturnsFalse()
        {
            // Arrange
            Directory.CreateDirectory(_tempDir);
            // Don't create UnityMcp.csproj
            LogAssert.Expect(LogType.Error, new Regex(@"\[MCP Unity\] UnityMcp\.csproj not found in server path:"));

            // Act
            bool result = McpUtils.ValidateServerPath(_tempDir);

            // Assert
            Assert.IsFalse(result, "ValidateServerPath should return false when UnityMcp.csproj is missing");
        }
