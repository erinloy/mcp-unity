...
namespace McpUnity.DirectMcp.Services
{
    ...
    public class UnityBridgeService : IHostedService
    {
        ...
        // 1 while a connect/retry loop is running; the reconnect timer and the connection-lost
        // handler must not start a second, overlapping loop.
        private int _connectLoopActive;
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            // MCP_UNITY_SETTINGS_PATH (written into generated client configs) takes precedence.
            var explicitSettingsPath = Environment.GetEnvironmentVariable("MCP_UNITY_SETTINGS_PATH");
            var settingsPath = string.IsNullOrWhiteSpace(explicitSettingsPath)
                ? Path.Combine(_unityProjectPath, "ProjectSettings", "McpUnitySettings.json")
            ...
            // IPv4 loopback by default (the Editor binds 127.0.0.1); UNITY_HOST targets a remote
            // Editor that has AllowRemoteConnections enabled.
            var host = Environment.GetEnvironmentVariable("UNITY_HOST");
            if (string.IsNullOrWhiteSpace(host))
            {
                host = "127.0.0.1";
            }

            _webSocketUri = $"ws://{host}:{port}/McpUnity";
        ...
        private async Task EnsureConnectedAsync()
        {
            ...
            if (Interlocked.Exchange(ref _connectLoopActive, 1) == 1)
            {
                return;
            }
            ...
            {
                Volatile.Write(ref _connectLoopActive, 0);
            }
        }
        ...
        private async Task ConnectWithRetriesAsync()
        {
            ...
            while (retryCount < maxRetries)
            {
                try
                {
                    ...
                    // Resolved on every attempt so a token created or regenerated by the Editor
                    // after this process started is picked up without a restart.
                    var authToken = UnityBridgeAuthentication.ResolveToken(_unityProjectPath!);
                    await _connectionManager.ConnectAsync(_webSocketUri!, authToken);
                ...
                catch (UnityBridgeAuthenticationException authEx)
                {
                    _logger.LogError("Unity bridge authentication failed (attempt {Attempt}): {Message}",
                        retryCount + 1, authEx.Message);
                    retryCount++;
